   ...
   <!-- 头部 end-->
	...
	<div class="">
		...
		<#if member?? && member.balancePwd?? && member.balancePwd != ''>
			<a href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/member/editbalancepassword.html" class="btn btn-default">修改支付密码</a>
			<a href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/member/resetbalancepassword.html" class="btn btn-default">重置支付密码</a>
		<#else>
			<a href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/member/setbalancepassword.html" class="btn btn-default">设置支付密码</a>
		</#if>
		</div>
...
$(function(){
	$("#balance_more_json").click(function(){
		...
		var urljson = "${(domainUrlUtil.EJS_URL_RESOURCES)!}/member/balanceJson.html?pageNum=" + list_page;
