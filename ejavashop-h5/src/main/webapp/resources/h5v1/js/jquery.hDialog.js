	...
	*/
	$.extend({  
        ...
        dialog: function (type, title, content, time, callback) {
    			...
    			okTpl = '<a href="javascript:;" style="float:left; width:47%; text-align: center; padding:5px 10px;display:inline-block;border-radius:3px;background-color:#ff7e18;color:#fff;" id="h_ok">'+ okText +'</a>',
    			cancelTpl = '<a href="javascript:;" style="float:right; width:47%; text-align: center; padding:5px 10px;display:inline-block;border-radius:3px;margin-left:10px;background-color:#eee;color:#333;" id="h_cancel">'+ cancelText +'</a>',
    			contentTpl = '<div id="hDialog-content" style="padding:15px 15px;text-align:left;">' + content + '</div>',
    			...
    			if(type === 'confirm'){ footerTpl = '<div id="hDialog-footer" style="padding:10px;border-top:1px solid #ddd;text-align:right; margin-bottom: 30px;">'+ okTpl + cancelTpl +'</div>'; }
            ...
            // $('a:contains('+cancelText+')').on("click", function() { $.closeDialog(); });
    			$('#h_ok').on("click", function() { if(callback != undefined){ methods.fire.call(this, callback); } });
                $('#h_cancel').on("click", function() { $.closeDialog(); });
        },
        /**
		 * @decription 关闭消息框
		 * @param callback : 回调函数
