(function($){
	$.fn.changeImg=function(options){
		...
		var $changeBox=$that.find(".focus-img-con"),
			...
			$changeLi=$changeBox.find(".licss-list"),
			$changePrev='',
			$changeNext='',
		...
		$changePrev.on("click",function(){
			if(_handle){
				...
				for(var i=0; i<ops.changeLen; i++){
					var _li=$changeUl.find(".licss-list").eq(i).clone(true);
					$changeUl.append(_li);
				};
				$changeUl.stop().animate({
					...
					for(var i=0; i<ops.changeLen; i++){
						$changeUl.find(".licss-list").eq(0).remove();
					};
					$changeUl.css('left',0);
					_handle=true;
				});
		...
		$changeNext.on("click",function(){
			...
			if(_handle){
				...
				for(var i=0; i<ops.changeLen; i++){
					var $_li=$changeUl.find(".licss-list").eq(_len1).clone(true);
				...
				$changeUl.stop().animate({
					...
					for(var i=0; i<ops.changeLen; i++){
						$changeUl.find(".licss-list").eq(_len).remove();
					};
					_handle=true;
				});
				/*autoPlay();*/
