...
//                System.out.println("############################" + vcr.getVerifyCode());
    ...
    public @ResponseBody HttpJsonResult<Integer> sendVerifySMS(HttpServletRequest request,
        ...
        try {
            ...
            ServiceResult<List<Member>> serviceResult = memberService.getMemberByName(mob);
            if (!serviceResult.getSuccess()) {
                return new HttpJsonResult<Integer>("用户名校验出错，请重试");
            }
            if (serviceResult.getResult() != null && serviceResult.getResult().size() > 0) {
                return new HttpJsonResult<Integer>("用户名重复，请重新输入");
            }
...
//        return "h5v1/member/register";
    ...
    public @ResponseBody HttpJsonResult<Boolean> signupSubmit(HttpServletRequest request,
        ...
        if (verify_user == null || !verify_user.equals(userName)) {
        	return new HttpJsonResult<Boolean>("用户名称与发送短信验证码的手机号不一致！");
        }
        ...
        if (memberType != null && memberType.equals("2")) {
            ...
            */
            ...
            BackUrl = "/storeregister/apply.html";
