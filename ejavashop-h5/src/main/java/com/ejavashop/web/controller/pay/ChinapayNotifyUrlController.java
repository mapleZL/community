...
//                String accNo = resultMap.get("OrderStatus"); //获取交易卡号
                ...
                if (EjavashopConfig.CHINAPAY_SUCCESS_STATUS.equals(resultMap.get("OrderStatus"))) {
                    //                    BigDecimal settleAmtB = new BigDecimal(settleAmt);
                    //                    settleAmtB = settleAmtB.divide(new BigDecimal(100));
                    //
                    //                    ServiceResult<Boolean> orderPayResult = ordersService.orderPayAfter(orderId, settleAmtB.toString(), "chinapay", "银联支付", accNo, resultMap.toString());
...
//                OrdersTradeSerial ots = ordersTradeSerialService.getOrdersTradeSerialById(tradesn)
//                    .getResult();
