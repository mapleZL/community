...
<script language="javascript">
	...
	//为客户端装配本页面需要的字典数据,多个用逗号分隔
		...
		$("#a-gridAdd").click(function(){
	 		window.location.href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/promotion/coupon/add";
		});
		...
		$("#a-gridEdit").click(function(){
            ...
            if(selected.status != 1 && selected.status != 4) {
	 		...
	 		window.location.href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/promotion/coupon/edit?couponId="+selected.id;
		});
		...
		$("#a-gridAudit").click(function(){
            ...
            if(selected.status != 2) {
            	$.messager.alert('提示','非提交审核的红包不能执行审核操作。');
                return;
            }
	 		window.location.href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/promotion/coupon/audit?couponId="+selected.id;
		});
		
		$("#a-gridAudit2").click(function(){
	        var selected = $('#dataGrid').datagrid('getSelected');
	        if(!selected){
	            $.messager.alert('提示','请选择操作行。');
	            return;
	        ...
	        $.messager.confirm('提示', '提价审核后红包不能再修改，请确认红包信息填写正确，确定提交审核该红包吗？', function(r){
	            if (r){
	                ...
	                $.ajax({
	                    type:"POST",
	                    url: "${domainUrlUtil.EJS_URL_RESOURCES}/admin/promotion/coupon/audit2",
	                    dataType: "json",
	                    data: "id="+selected.id,
	                    cache:false,
	    ...
	    $("#a-gridUp").click(function(){
	        ...
	        $.messager.confirm('提示', '确定上架该红包吗？', function(r){
	            if (r){
	                ...
	                $.ajax({
	                    type:"POST",
	                    url: "${domainUrlUtil.EJS_URL_RESOURCES}/admin/promotion/coupon/up",
	                    dataType: "json",
	                    data: "id="+selected.id,
	                    cache:false,
	                    success:function(data, textStatus){
	    ...
	    $("#a-gridDown").click(function(){
	        ...
	        $.messager.confirm('提示', '红包下架后将不能再次执行上架操作，确定下架该红包？', function(r){
	            if (r){
	                ...
	                $.ajax({
	                    type:"POST",
	                    url: "${domainUrlUtil.EJS_URL_RESOURCES}/admin/promotion/coupon/down",
	                    dataType: "json",
	                    data: "id="+selected.id,
	                    cache:false,
	                    success:function(data, textStatus){
		...
		$("#a-gridUserDetail").click(function(){
            ...
            if(selected.status != 5 && selected.status != 6) {
	 		...
	 		window.location.href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/promotion/couponuser?couponId="+selected.id;
		});
...
<div data-options="region:'center'" border="false">
	<table id="dataGrid" class="easyui-datagrid"
			data-options="rownumbers:true
						...
						,pageSize:'${pageSize}'
						,fit:true
    					,url:'${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/promotion/coupon/list'
	...
	<div id="gridTools">
		...
		<@shiro.hasPermission name="/admin/promotion/coupon/add">
		<a id="a-gridAdd" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true">新增</a>
		</@shiro.hasPermission>
		<@shiro.hasPermission name="/admin/promotion/coupon/edit">
		<a id="a-gridEdit" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true">修改</a>
		</@shiro.hasPermission>
		<@shiro.hasPermission name="/admin/promotion/coupon/delete">
		<a id="a-gridDel" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-delete" plain="true">删除</a>
		</@shiro.hasPermission>
		<@shiro.hasPermission name="/admin/promotion/coupon/audit">
		<a id="a-gridAudit2" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true">提交审核</a>
		</@shiro.hasPermission>
		<@shiro.hasPermission name="/admin/promotion/coupon/up">
		<a id="a-gridUp" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true">上架</a>
		</@shiro.hasPermission>
		<@shiro.hasPermission name="/admin/promotion/coupon/down">
		<a id="a-gridDown" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true">下架</a>
		</@shiro.hasPermission>
		<@shiro.hasPermission name="/admin/promotion/coupon/audit">
		<a id="a-gridAudit" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true">审核</a>
		</@shiro.hasPermission>
		<@shiro.hasPermission name="/admin/promotion/couponuser">
