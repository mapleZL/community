...
$(function(){
	...
	$("#back").click(function(){
 		window.location.href="${domainUrlUtil.EJS_URL_RESOURCES}/admin/mindex/floordata";
	});
	$("#edit").click(function(){
		...
		if($("#addForm").form('validate')){
	 		$("#addForm").attr("action", "${domainUrlUtil.EJS_URL_RESOURCES}/admin/mindex/floordata/update")
  				 .attr("method", "POST")
  				 .submit();
  		}
	});
	...
	$('#pro').click(function(){
		...
		$('#gd_dataGrid').datagrid({
			url:'${domainUrlUtil.EJS_URL_RESOURCES}/admin/product/listnopage?channel=h5floordata',
...
<div class="wrapper">
	<div class="formbox-a">
		<h2 class="h2-title">编辑楼层数据<span class="s-poar"><a class="a-back" href="${domainUrlUtil.EJS_URL_RESOURCES}/admin/mindex/floordata">返回</a></span></h2>
		...
		<div class="form-contbox">
			...
			<dl class="dl-group">
				...
				<dd class="dd-group">
					<input type="hidden" id="id" name="id" value="${(mIndexFloorData.id)!''}">
					<div class="fluidbox">
						<p class="p12 p-item">
							<label class="lab-item"><font class="red">*</font>楼层：</label>
							<select name="indexFloorId" id="indexFloorId" value="${(mIndexFloorData.indexFloorId)!''}" disabled="disabled">
					...
					<br/>
					<div id="dataPrdDiv" <#if mIndexFloorData?? && mIndexFloorData.dataType?? && mIndexFloorData.dataType != 1>style="display:none;"</#if>>
					<div class="fluidbox">
						<p class="p12 p-item">
					...
					<div id="dataLinkDiv" <#if mIndexFloorData?? && mIndexFloorData.dataType?? && mIndexFloorData.dataType != 2>style="display:none;"</#if>>
					<div class="fluidbox">
						<p class="p12 p-item">
					...
					<div class="fluidbox">
						<p class="p12 p-item">
							...
							<input type="hidden" id="image" name="image" value="${(mIndexFloorData.image)!''}">
						</p>
						<p class="p12 p-item">
							<label class="lab-item">&nbsp;</label>
							<label>
						...
						<p class="p12 p-item">
							<label class="lab-item">&nbsp;</label>
							<img alt="图片" style="width: 144px;height: 144px;" src="${domainUrlUtil.EJS_IMAGE_RESOURCES}${(mIndexFloorData.image)!''}">
						</p>
					</div>
					<br/>
					<br/>
