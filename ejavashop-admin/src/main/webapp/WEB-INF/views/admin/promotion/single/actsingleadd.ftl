...
$(function(){
	...
	$("#back").click(function(){
 		window.location.href="${domainUrlUtil.EJS_URL_RESOURCES}/admin/promotion/single";
	});
	$("#add").click(function(){
		...
		if($("#addForm").form('validate')){
	    	...
	    	if (type == 1) {
	    		...
	    		if(discount < 1){
	    			$.messager.alert('提示', '限购数量必须大于1。');
					return;
	    		}
	    		var str = (discount+'').split(".");
	    		var last_cell = str[1];
	    		if(parseInt(last_cell) > 0){
	    			$.messager.alert('提示', '限购数量必须为整数。');
					return;
	    		}
	    	}
			
	 		$("#addForm").attr("action", "${domainUrlUtil.EJS_URL_RESOURCES}/admin/promotion/single/create")
  				 .attr("method", "POST")
  				 .submit();
  		}
	});
...
<div class="wrapper">
	<div class="formbox-a">
		<h2 class="h2-title">添加单品立减<span class="s-poar"><a class="a-back" href="${domainUrlUtil.EJS_URL_RESOURCES}/admin/promotion/single">返回</a></span></h2>
		...
		<div class="form-contbox">
			...
			<dl class="dl-group">
				...
				<dd class="dd-group">
					...
					<div class="fluidbox">
						<p class="p12 p-item">
							<label class="lab-item"><font class="red">*</font>优惠额/折扣/限购数量：</label>
						...
						<p class="p12 p-item">
							...
							<label>
								...
								活动类型为减免金额时为金额（如10为减免10元），折扣类型时为折扣（如0.90为打九折），折扣类型为限购时为限购数量（单位为手，如每人限购10手）。
								</font>
							</label>
						</p>
					</div>
					...
					<input type="hidden" id="actSingleId" name="actSingleId" value="${(actSingle.id)!''}">
					<div class="fluidbox">
						<p class="p12 p-item">
					...
					<#if productList??>
						<#list  productList as product>
							<div class="fluidbox addItemT">
									<label class="lab-item">&nbsp;</label>
									<input type="hidden" id="ids" name="ids" value="${(product.seller_id)!''}:${(product.id)!''}" />
									商品名称：
