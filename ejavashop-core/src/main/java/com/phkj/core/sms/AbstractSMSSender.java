...
import com.phkj.core.sms.bean.VerifyCodeResult;
...
public abstract class AbstractSMSSender {
     ...
     * @return
     */
    public VerifyCodeResult sendSMSYP() {
        log.debug("验证码：" + this.verifyCode);
        try {
            String result = "";
            if (!"".equals(this.content)) 
                result = JavaAmsApi.sendResetCode(this.content, this.mobile);
            else
                result = JavaAmsApi.sendValidCode(this.verifyCode, this.mobile);
            Gson gson = new Gson();
