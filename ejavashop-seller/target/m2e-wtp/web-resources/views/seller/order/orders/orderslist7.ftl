...
<script language="javascript">
	...
	$(function() {
	    ...
	    $('#btn-add').click(function(){
	      ...
	      window.location.href="${currentBaseUrl}/applysendgoods?orderId="+selectedCode.id;
	    });
	    ...
	    $('#btn-return').click(function(){
	        ...
	        if(!selectedCode){
	         ...
	         $.messager.confirm('Confirm', '您确定要退货吗？', function(r){
	               if (r) {
	                   $.messager.progress({text:"提交中..."});
	                   $.ajax({
	                        type:"GET",
                            url: "${currentBaseUrl}/returninstore?id="+selectedCode.id,
	                        success:function(e) {
	                             $('#dataGrid').datagrid('reload',queryParamsHandler());
...
<div data-options="region:'center'" border="false">
	<table id="dataGrid" class="easyui-datagrid"
		data-options="rownumbers:true
						...
						,pageSize:'${pageSize}'
						,fit:true
    					,url:'${currentBaseUrl}/listInStore'
		...
		<thead>
			<tr>
				...
				-->
				<th field="deliveredSum" width="80" align="left">已发货数量</th>
	...
	<div id="gridTools">
	    <@shiro.hasPermission name="/seller/order/orders/listInStore">
		<a id="btn-search" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-search" plain="true">查询</a>
		</@shiro.hasPermission>
		<@shiro.hasPermission name="/seller/order/orders/applysendgoods">
		<a id="btn-add" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true">云仓托管发货</a>
		</@shiro.hasPermission>
		<@shiro.hasPermission name="/seller/order/orders/returninstore">
		<a id="btn-return" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-delete" plain="true">退货</a>
	     </@shiro.hasPermission>
	</div>
</div>
