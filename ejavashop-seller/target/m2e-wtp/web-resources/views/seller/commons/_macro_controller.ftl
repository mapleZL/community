...
<#macro radio id name="" value="" codeDiv="" class="rad" isDefault = "true" >
	...
	<#if codeManager.codeMap[codeDiv]?? && codeManager.codeMap[codeDiv]?size &gt; 0>
		<#list codeManager.codeMap[codeDiv] as code>
			<label><input type="radio" id="${id}" name="${localName}" class="${class}" value="${code.codeCd}" <#if code.codeCd == value  || (isDefault == 'true' && value == '' && code_index == 0)> checked </#if> /> ${code.codeText}</label>
		</#list>	
	</#if>
</#macro>
<#macro radio1 id name="" value="" codeDiv="" class="rad" isDefault = "true" >
	...
	<#if codeManager.codeMap[codeDiv]?? && codeManager.codeMap[codeDiv]?size &gt; 0>
		<#list codeManager.codeMap[codeDiv] as code>
			<label><input type="radio" id="${id}" disabled name="${localName}" class="${class}" value="${code.codeCd}" <#if code.codeCd == value  || (isDefault == 'true' && value == '' && code_index == 0)> checked </#if> /> ${code.codeText}</label>
		</#list>
	</#if>
</#macro>
...
<#macro radioonly id name="" value="" codeDiv="" class="rad"  >
	...
	<#list codeManager.codeMap[codeDiv] as code>
		<label><input type="radio" id="${id}" name="${localName}" class="${class}" value="${code.codeCd}" <#if code.codeCd == value || value == '' && code_index == 0> checked </#if>  disabled=""/> ${code.codeText}</label>
	</#list>	
</#macro>
<#-- 复选钮  -->
<#macro checkbox id name="" value=[] codeDiv="">
	...
	<#list codeManager.codeMap[codeDiv] as code>
		...
		<label><input type="checkbox" id="${id}" name="${localName}" value="${code.codeCd}" class="chk" ${checked} /> ${code.codeText}</label>
	</#list>	
</#macro>
