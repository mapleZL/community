...
function submitdata() {
	...
	if($("#checktype").val() == 2){
		var stone_step_1s = parseInt($("#stone_step_1s").val());
		var stone_step_1e = parseInt($("#stone_step_1e").val());
		var stone_step_2s = parseInt($("#stone_step_2s").val());
		var stone_step_2e = parseInt($("#stone_step_2e").val());
		var stone_step_3s = parseInt($("#stone_step_3s").val());
		var num = parseInt($("#number").val());
		if($("#price_step_1").val()=='' || $("#price_step_2").val()=='' || $("#price_step_3").val() == ''){
			$.messager.alert('提示',"阶梯价价格不能为空，请检查");
			return;
		}
	}
