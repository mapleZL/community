		...
		$('#btn_freeze').click(function(){
			...
			$.messager.confirm('确认', '确定冻结该管理员账号吗？冻结后，该管理员将无法登录', function(r){
				if (r){
					...
					$.ajax({
						type:"GET",
					    url: "${currentBaseUrl}/freeze.html?id="+selected.id,
						success:function(e){
		...
		$('#btn_unfreeze').click(function(){
			...
			$.messager.confirm('确认', '确定解冻该管理员账号吗？', function(r){
				if (r){
					...
					$.ajax({
						type:"GET",
					    url: "${currentBaseUrl}/unfreeze.html?id="+selected.id,
						success:function(e){
							$('#dataGrid').datagrid('reload');
							$.messager.progress('close');
							$.messager.show({
...
<div data-options="region:'center'" border="false">
	...
	<div id="gridTools">
		...
		<a id="btn_freeze" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true">冻结</a>
		</@shiro.hasPermission>
		<@shiro.hasPermission name="/seller/system/sellerUser/unfreeze">
		<a id="btn_unfreeze" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true">解冻</a>
		</@shiro.hasPermission>
