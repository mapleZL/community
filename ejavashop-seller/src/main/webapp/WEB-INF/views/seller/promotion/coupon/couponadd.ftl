	...
	$("#add").click(function(){
		...
		if($("#addForm").form('validate')){
			...
			if (parseFloat(minAmount) <= parseFloat(couponValue)) {
	 		...
	 		$("#addForm").attr("action", "${domainUrlUtil.EJS_URL_RESOURCES}/seller/promotion/coupon/create")
  				 .attr("method", "POST")
  				 .submit();
  		}
	});
...
<div class="wrapper">
	<div class="formbox-a">
		<h2 class="h2-title">添加红包<span class="s-poar"><a class="a-back" href="${domainUrlUtil.EJS_URL_RESOURCES}/seller/promotion/coupon">返回</a></span></h2>
