	...
	$("#edit").click(function(){
		...
		if($("#addForm").form('validate')){
			...
			if (channel == null || channel == "") {
	 		...
	 		$("#addForm").attr("action", "${domainUrlUtil.EJS_URL_RESOURCES}/seller/promotion/full/update")
  				 .attr("method", "POST")
  				 .submit();
  		}
	});
...
<div class="wrapper">
	<div class="formbox-a">
		<h2 class="h2-title">编辑订单满减<span class="s-poar"><a class="a-back" href="${domainUrlUtil.EJS_URL_RESOURCES}/seller/promotion/full">返回</a></span></h2>
		...
		<div class="form-contbox">
			...
			<dl class="dl-group">
				...
				<dd class="dd-group">
					<input type="hidden" id="id" name="id" value="${(actFull.id)!''}">
					<input type="hidden" id="status" name="status" value="${(actFull.status)!''}">
					...
					<div class="fluidbox">
						<p class="p12 p-item">
							...
							<input type="text" id="startTime" name="startTime"
								...
								onclick="WdatePicker({startDate:'%y-%M-{%d} 00:00:00',dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'endTime\')}'});"
