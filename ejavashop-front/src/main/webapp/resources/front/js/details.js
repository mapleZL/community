...
function preview(img){
	...
	$("#preview .jqzoom img").attr("src", val);
	$("#preview .jqzoom img").attr("jqimg", val);

	var bool = false;
	$(".items ul li").each(function(i) {
		var cVal = $(this).children().attr("src");
		if (cVal == val) {
			$(".jqzoom img").attr("src", cVal);
			$(".items ul li:eq(" + i + ") img").css("border", "2px solid #e4393c");
			$(".items ul li:not(:eq(" + i + ")) img").css("border", "1px solid #CCC");
			return false;
		}
	});
...
function leaveScroll() {
	detailTime = setTimeout(autoSlideDetail, 3000);
}

function clearAutoSlideDetail() {
	window.clearTimeout(detailTime);
	$(".items ul li img").css("border", "1px solid #CCC");
}

function autoSlideDetail() {
	var val = $(".jqzoom img").attr("src");
	var bool = false;
	var len = $(".items ul li").length;
	$(".items ul li").each(function(i) {
		var cVal = $(this).children().attr("src");
		if (bool) {
			$(".jqzoom img").attr("src", cVal);
			$(".jqzoom img").attr("jqimg", cVal);
			$(".items ul li:eq(" + i + ") img").css("border", "2px solid #e4393c");
			$(".items ul li:not(:eq(" + i + ")) img").css("border", "1px solid #CCC");
			return false;
		}
		if (cVal == val && i < (len - 1)) bool = true;
	});
	if (!bool) {
		$(".jqzoom img").attr("src", $(".items ul li:first img").attr("src"));
		$(".jqzoom img").attr("jqimg", $(".items ul li:first img").attr("src"));
		$(".items ul li:eq(0) img").css("border", "2px solid #e4393c");
		$(".items ul li:not(:eq(0)) img").css("border", "1px solid #CCC");
	}
	detailTime = setTimeout(autoSlideDetail, 3000);
}
...
$(function(){
	...
	//下一张
	$(".spec-scroll .next2").bind("click",function(){
	...
	//上一张
	$(".spec-scroll .prev2").bind("click",function(){
		if(tempLength > 0){
...
/*$("#add").click(function(){
    ...
    var productStock = $("#productStock").html();
    if(Number(t.val() > Number(productStock))){
