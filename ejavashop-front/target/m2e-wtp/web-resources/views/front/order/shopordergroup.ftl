						...
						<div id='payShipAndSkuInfo'>
							...
							<div class='shopping-lists'>
								<div class='order-common-list'>
									...
									<#if product?? && productGoods?? && actGroup?? >
									<!--  单品 -->
									<div class='goods-items '>
										<div class='goods-item goods-item-extra'>
											<div class='p-img'>
												<a href='${(domainUrlUtil.EJS_URL_RESOURCES)!}/tuan/${(actGroup.id)!0}.html' target="_blank">
													<img width="80" height="80" src='${(domainUrlUtil.EJS_IMAGE_RESOURCES)!}${(actGroup.image)!""}'>
												</a>
											</div>
											<div class='goods-msg'>
												<div class='goods-msg-gel'>
													<div class='p-name'>
														<a href='${(domainUrlUtil.EJS_URL_RESOURCES)!}/tuan/${(actGroup.id)!0}.html' target='_blank'>${product.name1!''} ${productGoods.normName!''}</a>
													</div>
													<div class='p-price'>
														<strong class=''>￥${(actGroup.price)?string('0.00')!0}</strong>
													</div>
													<div class='p-num'>
													...
													<div class='p-state'>
														<span>
															<#if (actGroup.stock)?? && (actGroup.stock &gt; number)>
																有货
															<#else>
													...
													<div class='p-total'>
														<strong>
															${(actGroup.price * number)?string('0.00')!0}
														</strong>
													</div>
												</div>
											</div>
			...
			<div class='statistic' style='float:right'>
				<div class='list'>
					<span>
					 ...
					 <em class='price' id='warePriceId'>￥${(actGroup.marketPrice * number)?string("0.00")!'0.00'}</em>
				</div>
				<div class='list'>
					<span>
						 总商品金额（团购）：
					</span>
					 <em class='price' id='discountAmountPriceId'>￥${(actGroup.price * number)?string("0.00")!'0.00'}</em>
				</div>
				<div class='list'>
					<span>活动节省 ：</span>
					<em class='price' id='discountPriceId'>
						<font style="color:#FF6600"> - ￥${((actGroup.marketPrice - actGroup.price) * number)?string("0.00")!'0.00'} </font>
					</em>
				</div>
				<div class='list'>
					<span>运费 ：</span>
				...
				<div class='list'>
					<span>应付总额 ：</span>
					<em class='price' id='sumPayPriceId'>￥${((actGroup.price * number) + transFee)?string("0.00")!'0.00'}</em>
					<!-- 记录一个hidden值方便计算 -->
					<input type="hidden" id="sumPayPriceHidden" value="${((actGroup.price * number) + transFee)?string('0.00')!'0.00'}" autocomplete="off"/>
				</div>
			</div>
		</div>
		<div class='clr'></div>
			...
			<div class='group' id='checkout-floatbar'>
				<div class='checkout-buttons'>
					<div class='sticky-wrap'>
						<div class='inner'>
							<#if actGroup??>
							<i class="act-info">团购订单必须在团购活动结束前支付，请及时支付您的订单，谢谢！</i>
							</#if>
							...
							<span class='total'>
								应付总额：
								<strong id='payPriceId'>￥${((actGroup.price * number) + transFee)?string("0.00")!'0.00'}</strong>
							</span>
						</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	function addressSelect(){
		$(".consignee-item").click(function(){
			...
			var price = ${(actGroup.price)?string("0.00")!'0.00'}
			var number = ${(number)!0};
			if (oldAddressId != newAddressId) {
				...
				$.ajax({
					...
					data : {sellerId:sellerId,addressId:newAddressId,number:number},
					dataType : "json",
	...
	function updateInvoice(obj,id){
  	...
  	$(function(){
		...
		$("#sumPayPriceHidden").val(${((actGroup.price * number) + transFee)?string("0.00")!'0.00'});
		$("#usedBalanceHidden").val(0);
		// 积分tips
	  	// $(".beans-cho-tip").tips({
	   //  	trigger: '.qmark-tip',
