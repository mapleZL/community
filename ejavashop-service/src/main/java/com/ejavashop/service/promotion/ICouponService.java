package com.ejavashop.service.promotion;
...
public interface ICouponService {
     ...
     * @return
     */
    ServiceResult<List<CouponUser>> getCouponUserByMemberIdAndUse(Integer memberId, PagerInfo pager,
                                                                  Integer canUse);
    ...
    ServiceResult<List<CouponUser>> getEffectiveByMemberIdAndSellerId(Integer memberId,
                                                                      Integer sellerId);
    /**
     * 根据用户ID、商家ID、商品总金额获取当前时间有效可用的优惠券【仝照美】
     * @param queryMap
     * @param pager
     * @return
     */
    ServiceResult<List<CouponUser>> getEffectiveByMemberIdAndSellerIdAndTotalPrice(Integer memberId,
    		Integer sellerId,BigDecimal totalPrice);
    ...
    /**
     * 通过总商品金额  在表coupon（ID在17和23之间） 匹配一条 红包规则 
     * @param couponSn
     * @return
     */
	ServiceResult<List<Coupon>> getCouponsByPrice(BigDecimal totalPrice);
