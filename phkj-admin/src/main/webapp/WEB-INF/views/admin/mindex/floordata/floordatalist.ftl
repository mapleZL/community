...
<script language="javascript">
	...
	//为客户端装配本页面需要的字典数据,多个用逗号分隔
		...
		$("#a-gridAdd").click(function(){
	 		window.location.href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/mindex/floordata/add";
		});
		...
		$("#a-gridEdit").click(function(){
			...
			if(!selected) {
	 		...
	 		window.location.href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/mindex/floordata/edit?mIndexFloorDataId="+selected.id;
		});
		...
		$("#a-gridDel").click(function(){
            ...
            $.messager.confirm('提示', '确定删除吗？', function(r){
                if (r){
                    ...
                    $.ajax({
                        type:"POST",
                        url: "${domainUrlUtil.EJS_URL_RESOURCES}/admin/mindex/floordata/delete",
                        dataType: "json",
...
<div data-options="region:'center'" border="false">
	<table id="dataGrid" class="easyui-datagrid"
			data-options="rownumbers:true
						...
						,pageSize:'${pageSize}'
						,fit:true
    					,url:'${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/mindex/floordata/list'
