...
<script language="javascript">
	...
	//为客户端装配本页面需要的字典数据,多个用逗号分隔
		<#noescape>
			...
			recommendBox = eval('(${initJSCodeContainer("RECOMMEND_TYPE")})');
		</#noescape>

		$("#a-gridAdd").click(function(){
	 		window.location.href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/pcindex/recommend/add";
		});
		...
		$("#a-gridEdit").click(function(){
			...
			if(!selected) {
	 		...
	 		window.location.href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/pcindex/recommend/edit?pcRecommendId="+selected.id;
		});
		...
		$("#a-gridDel").click(function(){
            ...
            $.messager.confirm('提示', '确定删除吗？', function(r){
                if (r){
                    ...
                    $.ajax({
                        type:"POST",
                        url: "${domainUrlUtil.EJS_URL_RESOURCES}/admin/pcindex/recommend/delete",
                        dataType: "json",
		...
		$("#a-gridUp").click(function(){
	        ...
	        $.messager.confirm('提示', '确定启用吗？', function(r){
	            if (r){
	                ...
	                $.ajax({
	                    type:"POST",
	                    url: "${domainUrlUtil.EJS_URL_RESOURCES}/admin/pcindex/recommend/up",
	                    dataType: "json",
	                    data: "id="+selected.id,
	                    cache:false,
	                    success:function(data, textStatus){
		...
		$("#a-gridPre").click(function(){
	        ...
	        $.messager.confirm('提示', '确定预使用吗？', function(r){
	            if (r){
	                ...
	                $.ajax({
	                    type:"POST",
	                    url: "${domainUrlUtil.EJS_URL_RESOURCES}/admin/pcindex/recommend/pre",
	                    dataType: "json",
	                    data: "id="+selected.id,
	                    cache:false,
	                    success:function(data, textStatus){
	    ...
	    $("#a-gridDown").click(function(){
	        ...
	        $.messager.confirm('提示', '确定停用吗？', function(r){
	            if (r){
	                ...
	                $.ajax({
	                    type:"POST",
	                    url: "${domainUrlUtil.EJS_URL_RESOURCES}/admin/pcindex/recommend/down",
	                    dataType: "json",
	                    data: "id="+selected.id,
	                    cache:false,
	                    success:function(data, textStatus){
	...
	function recommendFormat(value,row,index){
		return recommendBox["RECOMMEND_TYPE"][value];
	}
...
<div data-options="region:'center'" border="false">
	<table id="dataGrid" class="easyui-datagrid"
			data-options="rownumbers:true
						...
						,pageSize:'${pageSize}'
						,fit:true
    					,url:'${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/pcindex/recommend/list'
