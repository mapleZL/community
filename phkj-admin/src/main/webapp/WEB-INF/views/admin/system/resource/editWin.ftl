<#assign
currentBaseUrl="${domainUrlUtil.EJS_URL_RESOURCES}/admin/system/resource"/>
					...
					<div class="fluidbox">
						<p class="p12 p-item">
							...
							<select id="type" class="txt w200 easyui-combobox" 
								...
								<option value="2" <#if res??&&res.type==2>selected</#if>>按钮</option>
								<option value="1" <#if res??&&res.type==1>selected</#if>>菜单</option>
							</select>
						</p>
					</div>
					...
					<div class="fluidbox">
						<p class="p12 p-item">
							...
							<select id="scope" class="txt w200 easyui-combobox" name="scope"
								editable="false" data-options="required:true">
								<option value="">请选择</option>
								<option value="2" <#if res??&&res.scope==2>selected</#if>>平台</option>
								<option value="1" <#if res??&&res.scope==1>selected</#if>>商家</option>
							</select>
						</p>
					</div>
					...
					<div class="fluidbox">
						<p class="p12 p-item">
							...
							<input class="txt w200 easyui-validatebox" type="text" id="resIcon"
								name="resIcon" value="${res.resIcon!}"
								data-options="required:false,validType:'length[0,20]'" />
								<div class="tooltip tooltip-right"
									style="margin-left: 360px; display: block; color: #C93; background-color: #FFC; border-color: #DBB46F;">
