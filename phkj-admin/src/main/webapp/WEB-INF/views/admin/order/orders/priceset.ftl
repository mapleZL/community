...
<script language="javascript">
	...
	function submitdata(submitType){
		...
		if ($("#addForm").form('validate')) {
			if(submitType == "add"){
				...
				submitOrders(submitType);
			}
			if(submitType == "newOrders"){
				$("#newOrders").attr("disabled",true);
				$.messager.confirm('Confirm', '该操作将生成待发货的新的订单号，原订单将被取消，您确定继续该操作？', function(r){
                if (r) {
					submitOrders(submitType);
                }else{
                $("#newOrders").attr("disabled",false);
				return;
                }
                });
			}
	...
	function submitOrders(submitType){
	     $('#addForm').ajaxSubmit({
			            url: '${currentBaseUrl}/setPriceSvae?submitType='+submitType,
			            type: 'post',
		...
		<div class="form-contbox">
				...
				<#--
					...
					<div class="fluidbox">
						<p class="p12 p-item">
							...
							<label>&yen;${(orders.moneyDiscount)}</label>
						</p>
					</div>
					  <div class="fluidbox">
                        <p class="p12 p-item">
									...
									<tr style="border: 1px solid #e2e1e1" name="opinfostr">
										...
										<td>
											<#if (op.productPackageId)?? && (op.productPackageId) != 0>
												...
												<input class="easyui-numberbox stylePrice" name="packageFee" 
													data-options="min:0.00,max:99999,precision:2"
										...
										<td>
											<input type="text" protectedPrice="${(op.protectedPrice)!0}"
												onchange="opamountChange(this)" name="productAmount"
			...
			<p class="p-item p-btn">
				<input type="button" id="add" class="btn" value="确认"
					...
					<#if orders ?? && orders.orderState == 3>
					<input type="button" id="newOrders"
					class="btn" value="生成新订单"  onclick="submitdata('newOrders')"/> 
					</#if>
