...
<script language="javascript">
	...
	$(function() {
		...
		$('#a-returngoods').click(function(){
			...
			$.messager.prompt('确认','您确定要退货吗,如果该订单存在关联订单会一并被取消？', function(r) {
				var bool1 = (r=="");
	    ...
	    //部分退货（即次品退货）
	    $('#a-returngoods-1').click(function(){
	    	var selectedCode = $('#dataGrid').datagrid('getSelected');
	        if(!selectedCode){
	                $.messager.alert('提示','请选择操作行。');
	                return;
	         }
	         location.href='${currentBaseUrl}/setNumber?orderId='+selectedCode.id;
	    });
...
<div data-options="region:'center'" border="false">
	...
	<div id="gridTools">
		...
		<a id="a-returngoods-1" href="/admin/order/orders/setNumber" class="easyui-linkbutton" iconCls="icon-delete" plain="true">部分退货</a>
