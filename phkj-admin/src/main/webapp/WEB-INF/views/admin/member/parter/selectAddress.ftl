...
<script type="text/javascript">
	$(function(){
		$("#resourceTree").tree({
			...
			url: "${currentBaseUrl}/parterAddressTree?parterSignId=${parterSignId}"
		});
		...
		$('#saveRuleRes').click(function() {
			...
			$("#roleResForm").form('submit', {
				url : "${currentBaseUrl}/saveParterRegions",
				success : function(e) {
...
<form id="roleResForm" method="post">
	<div style="margin-top: 5px;">
		<label class="lab-item">合伙人: </label><span id="memberNameSpan">${memberName!}</span>(<label style="color: red; font-weight: bold;">新分配的功能及菜单需要重新登录后才能生效
		</label> )<br/><br/>
		<label style="color: green; font-weight: bold;margin-left: 15px;">已选择区域：${regionsName}
		</label> 
				<input id="resIds" type="hidden" name="resIds"> 
				<input id="parterSignId" type="hidden" name="parterSignId" value="${parterSignId!}">
				<input id="memberName" type="hidden" name="memberName" value="${memberName!}">
	</div>
