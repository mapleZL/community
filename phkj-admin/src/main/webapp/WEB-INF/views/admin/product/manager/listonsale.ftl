...
<script language="javascript">
	$(function(){
		$("#a_pro_editstock").click(function(){
			var selected = $('#dataGrid').datagrid('getSelected');
			if(!selected) {
				$.messager.alert('提示','请选择操作行。');
				return;
			}
	 		window.location.href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/productbuystock/editstock?id="+selected.id;
		});
		
		//导出按钮
		$("#a-gridDoExport").click(function(){
			$.messager.confirm('提示', '确定导出商品价格吗？', function(r){
	            if (r){
	            	location.href = "${currentBaseUrl}/exportOnsalesProductPrice";
	            	//$.fileDownload('${currentBaseUrl}/exportOnsalesProductPrice',{data:null});
	            }
	        });
		});
...
<#--3.function button----------------->
    <div id="gridTools">
        ...
        <@shiro.hasPermission name="/admin/productbuystock/editstock">
        <a id="a_pro_editstock" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true">设置下单限制</a>
        </@shiro.hasPermission>
