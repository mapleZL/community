...
<script language="javascript">
	...
	//为客户端装配本页面需要的字典数据,多个用逗号分隔
		...
		$("#a-gridAdd").click(function(){
	 		window.location.href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/promotion/single/add";
		});
		...
		$("#a-gridEdit").click(function(){
            ...
            if(selected.status != 1 && selected.status != 4) {
	 		...
	 		window.location.href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/promotion/single/edit?actSingleId="+selected.id;
		});
		$("#a-gridDetail").click(function(){
			...
			if(!selected) {
	 		...
	 		window.location.href="${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/promotion/single/detail?actSingleId="+selected.id;
		});
		...
		$("#a-gridDel").click(function(){
            ...
            $.messager.confirm('提示', '确定删除吗？', function(r){
                if (r){
                    ...
                    $.ajax({
                        type:"POST",
                        url: "${domainUrlUtil.EJS_URL_RESOURCES}/admin/promotion/single/delete",
                        dataType: "json",
		...
		$("#a-gridAudit").click(function(){
	        ...
	        $.messager.confirm('提示', '提价审核后活动不能再修改，请确认活动信息填写正确，确定提交审核该活动吗？', function(r){
	            if (r){
	                ...
	                $.ajax({
	                    type:"POST",
	                    url: "${domainUrlUtil.EJS_URL_RESOURCES}/admin/promotion/single/audit",
	                    dataType: "json",
	                    data: "id="+selected.id,
	                    cache:false,
	                    success:function(data, textStatus){
		...
		$("#a-gridUp").click(function(){
	        ...
	        $.messager.confirm('提示', '如果同一个商品同时间有多个单品立减活动上架，则会使用最新的一个有效活动，确定上架该活动吗？', function(r){
	            if (r){
	                ...
	                $.ajax({
	                    type:"POST",
	                    url: "${domainUrlUtil.EJS_URL_RESOURCES}/admin/promotion/single/up",
	                    dataType: "json",
	                    data: "id="+selected.id,
	                    cache:false,
	                    success:function(data, textStatus){
	    ...
	    $("#a-gridDown").click(function(){
	        ...
	        $.messager.confirm('提示', '活动下架后将不能再次执行上架操作，确定下架该活动？', function(r){
	            if (r){
	                ...
	                $.ajax({
	                    type:"POST",
	                    url: "${domainUrlUtil.EJS_URL_RESOURCES}/admin/promotion/single/down",
	                    dataType: "json",
	                    data: "id="+selected.id,
	                    cache:false,
	                    success:function(data, textStatus){
...
<div data-options="region:'center'" border="false">
	<table id="dataGrid" class="easyui-datagrid"
			data-options="rownumbers:true
						...
						,pageSize:'${pageSize}'
						,fit:true
    					,url:'${(domainUrlUtil.EJS_URL_RESOURCES)!}/admin/promotion/single/list2'
