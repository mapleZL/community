...
public interface CouponUserReadDao {
    ...
    /**
     * 根据用户ID、商家ID、商品总金额获取当前时间有效可用的优惠券
     * @param memberId
     * @param sellerId
     * @return
     */
	List<CouponUser> getEffectiveByMemberIdAndSellerIdAndTotalPrice(
			@Param("memberId") Integer memberId, @Param("sellerId") Integer sellerId, @Param("totalPrice") BigDecimal totalPrice);
}