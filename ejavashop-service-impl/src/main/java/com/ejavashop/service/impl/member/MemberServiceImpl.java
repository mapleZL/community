...
public class MemberServiceImpl implements IMemberService {
	...
	@Override
	public ServiceResult<List<Member>> getTuijianMemberByMemberId(
			Integer memberId) {
		ServiceResult<List<Member>> serviceResult = new ServiceResult<List<Member>>();
        try {
            serviceResult.setResult(memberModel.getTuijianMemberByMemberId(memberId));
        } catch (BusinessException be) {
            serviceResult.setSuccess(false);
            serviceResult.setMessage(be.getMessage());
        } catch (Exception e) {
	...
	@Override
	public ServiceResult<List<Member>> getParterTuijianByMemberId1(
			Integer memberId, String memberTuijianId) {
		ServiceResult<List<Member>> serviceResult = new ServiceResult<List<Member>>();
        try {
            serviceResult.setResult(memberModel.getParterTuijianByMemberId1(memberId,memberTuijianId));
        } catch (BusinessException be) {
            serviceResult.setSuccess(false);
            serviceResult.setMessage(be.getMessage());
        } catch (Exception e) {
	...
	@Override
	public ServiceResult<Boolean> transferBussiness(String memberId, String type) {
		ServiceResult<Boolean> serviceResult = new ServiceResult<Boolean>();
        try {
            serviceResult.setResult(memberModel.transferBussiness(memberId, type));
        } catch (BusinessException be) {
            serviceResult.setSuccess(false);
            serviceResult.setMessage(be.getMessage());
        } catch (Exception e) {
            serviceResult.setError(ConstantsEJS.SERVICE_RESULT_CODE_SYSERROR, "服务异常，请联系系统管理员。");
            log.error("[FrontMemberExtendService][memberId][type]设置oms账号权限是出现异常:", e);
        }
        return serviceResult;
	}
}