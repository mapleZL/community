...
public class SellerServiceImpl implements ISellerService {
	...
	@Override
	public ServiceResult<Boolean> checkSellerCode(String sellerCode) {
		ServiceResult<Boolean> serviceResult = new ServiceResult<Boolean>();
        try {
            serviceResult.setResult(sellerModel.checkSellerCode(sellerCode));
        } catch (BusinessException e) {
            serviceResult.setSuccess(false);
            serviceResult.setMessage(e.getMessage());
            log.error("[SellerService][checkSellerCode]根据商户编码获取商家表时出现异常：" + e.getMessage());
        } catch (Exception e) {
            serviceResult.setError(ConstantsEJS.SERVICE_RESULT_CODE_SYSERROR,
                ConstantsEJS.SERVICE_RESULT_EXCEPTION_SYSERROR);
            log.error("[SellerService][checkSellerCode] param1:" + sellerCode);
            log.error("[SellerService][checkSellerCode]根据商户编码获取商家表时出现未知异常：", e);
        }
        return serviceResult;
	}

    @Override
    public ServiceResult<List<Seller>> getSellerListAll() {
        ServiceResult<List<Seller>> serviceResult = new ServiceResult<List<Seller>>();
        try {
            serviceResult.setResult(sellerModel.getSellerListAll());
        } catch (Exception e) {
            serviceResult.setSuccess(false);
            serviceResult.setMessage(e.getMessage());
            log.error("获取商家表时出现异常：" + e.getMessage());
        }
        return serviceResult;
    }
}