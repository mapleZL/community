...
import com.ejavashop.model.member.MemberCollectionProductModel;
...
public class MemberCollectionProductServiceImpl implements IMemberCollectionProductService {
    ...
    */
    ...
    public ServiceResult<Integer> countCollect(Map<String, Object> queryMap, Integer memberId) {
        ServiceResult<Integer> result = new ServiceResult<Integer>();
        try {
            result.setResult(memberCollectionProductModel.countCollectionByMemberId(queryMap, memberId));
        } catch (Exception e) {
            log.error("根据id[" + memberId + "]取得会员收藏商品表时出现未知异常：" + e);
            result.setSuccess(false);
            result.setMessage("根据id[" + memberId + "]取得会员收藏商品表时出现未知异常");
        }
        return result;
    }
     ...
     */
    @Override
    public ServiceResult<List<FrontMemberCollectionProductVO>> getCollectionProductByMemberId(Map<String, Object> queryMap, Integer memberId, PagerInfo pager) {
     ...
     */
    @Override
    public ServiceResult<MemberCollectionProduct> saveMemberCollectionProduct(Integer productId, Integer memberId) {
        ServiceResult<MemberCollectionProduct> serviceResult = new ServiceResult<MemberCollectionProduct>();
        try {
            serviceResult.setResult(memberCollectionProductModel.saveMemberCollectionProduct(productId, memberId));
        } catch (BusinessException be) {
            serviceResult.setSuccess(false);
            serviceResult.setMessage(be.getMessage());
        } catch (Exception e) {
     ...
     */
    @Override
    public ServiceResult<MemberCollectionProduct> cancelCollectionProduct(Integer productId, Integer memberId) {
        ServiceResult<MemberCollectionProduct> serviceResult = new ServiceResult<MemberCollectionProduct>();
        try {
            serviceResult.setResult(memberCollectionProductModel.cancelCollectionProduct(productId, memberId));
        } catch (BusinessException be) {
            serviceResult.setSuccess(false);
            serviceResult.setMessage(be.getMessage());
        } catch (Exception e) {
