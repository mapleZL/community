...
import com.ejavashop.model.pcindex.PcIndexFloorModel;
...
public class PcIndexFloorServiceImpl implements IPcIndexFloorService {
	...
	@Override
	public ServiceResult<String> sortAndInsert(String ids, String sorts, String floorCode) {
		
		ServiceResult<String> result = new ServiceResult<String>();
        try {
        	result.setResult(productOrderModel.insertProductOrder(ids, sorts, floorCode));
        	//result.setMessage(value);
        } catch (BusinessException be) {
            result.setSuccess(false);
            result.setMessage(be.getMessage());
            log.error(
                "[IPcIndexFloorService][productOrderModel]取得PC端首页楼层时发生异常:" + be.getMessage());
        } catch (Exception e) {
            //result.setError(ConstantsEJS.SERVICE_RESULT_CODE_SYSERROR, "服务异常，请联系系统管理员。");
            result.setSuccess(false);
            log.error("[IPcIndexFloorService][productOrderModel]取得PC端首页楼层时发生异常:", e);
        }
        return result;
	}
