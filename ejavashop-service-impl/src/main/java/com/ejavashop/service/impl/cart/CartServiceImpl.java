package com.ejavashop.service.impl.cart;
...
public class CartServiceImpl implements ICartService {
    ...
    @Override
    public ServiceResult<Boolean> addToCart(Cart cart) {
    ...
    public ServiceResult<CartInfoVO> getCartInfoByMId(Integer memberId,
        ...
        try {
            serviceResult.setResult(cartModel.getCartInfoByMId(memberId, memberAddress, source,
                useType,transportType));
        } catch (BusinessException be) {
    ...
    */
    ...
    public ServiceResult<Cart> getCartById(Integer cartId) {
        ...
        try {
            ...
            result.setSuccess(false);
            result.setMessage(be.getMessage());
            log.error("[CartService][getCartById]根据id[" + cartId + "]取得商城购物车时出现未知异常:"
                      + be.getMessage());
        } catch (Exception e) {
            result.setError(ConstantsEJS.SERVICE_RESULT_CODE_SYSERROR, "服务异常，请联系系统管理员。");
    ...
    @Override
    public ServiceResult<Integer> getCartNumberByMId(Integer memberId) {
        ServiceResult<Integer> result = new ServiceResult<Integer>();
        try {
            result.setResult(cartModel.getCartNumberByMId(memberId));
        } catch (BusinessException be) {
            result.setSuccess(false);
            result.setMessage(be.getMessage());
    ...
    public ServiceResult<Boolean> updateChecked(Integer memberId, Integer cartId, Integer checked) {
        ...
        try {
            ...
            result.setSuccess(false);
            result.setMessage(be.getMessage());
            log.error("[CartService][updateChecked]根据用户ID、购物车ID修改购物车的选中状态时出现未知异常:"
                      + be.getMessage());
        } catch (Exception e) {
            result.setError(ConstantsEJS.SERVICE_RESULT_CODE_SYSERROR, "服务异常，请联系系统管理员。");
    ...
    @Override
    public ServiceResult<List<Cart>> queryList(Map<String, Object> queryMap) {
        ServiceResult<List<Cart>> result = new ServiceResult<List<Cart>>();
        try {
            result.setResult(cartModel.queryList(queryMap));
        } catch (BusinessException be) {
            result.setSuccess(false);
            result.setMessage(be.getMessage());
        } catch (Exception e) {
	...
	@Override
	public ServiceResult<Boolean> selectPackage(Integer memberId) {
		ServiceResult<Boolean> result = new ServiceResult<Boolean>();
        try {
            result.setResult(cartModel.selectPackage(memberId));
        } catch (BusinessException be) {
            result.setSuccess(false);
            result.setMessage(be.getMessage());
            log.error("[CartService][selectPackage]根据用户ID查询购物车选中商品是否有二次加共出现位置异常:"
                      + be.getMessage());
        } catch (Exception e) {
            result.setError(ConstantsEJS.SERVICE_RESULT_CODE_SYSERROR, "服务异常，请联系系统管理员。");
            log.error("[CartService][selectPackage]根据用户ID查询购物车选中商品是否有二次加共出现位置异常:", e);
        }
        return result;
	}

    @Override
    public ServiceResult<String> checklimitations(Integer memberId, Integer productId, Integer productGoodsId,
                                                   Integer number) {
        ServiceResult<String> result = new ServiceResult<String>();
        try {
            result.setResult(cartModel.checklimitations(memberId,productId,productGoodsId,number)); 
        } catch (Exception e) {
            result.setSuccess(false);
            result.setMessage(e.getMessage());
            log.error("[CartService][checklimitations]获取限购商品数据时出现异常:" + e.getMessage());
        }
        return result;
    }
}