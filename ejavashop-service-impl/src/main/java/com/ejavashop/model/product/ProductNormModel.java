...
public class ProductNormModel {
    ...
    public ProductNorm getNormById(Integer id) {
        ...
        ProductNorm productNorm = productNormReadDao.getNormById(id);
        if (null != productNorm && null != productNorm.getId()) {
            List<ProductNormAttr> attrs = productNormReadDao.getAttrByNormId(productNorm.getId());
    ...
    public List<ProductNorm> getNormByIds(String ids) {
        ...
        return productNormReadDao.getNormByIds(ids);
    }

    public List<ProductNorm> pageNorm(Map<String, String> queryMap, PagerInfo pager) {
        Integer start = 0, size = 0;
        if (pager != null) {
            pager.setRowsCount(productNormReadDao.countNorm(queryMap));
            start = pager.getStart();
            size = pager.getPageSize();
        }
        List<ProductNorm> list = productNormReadDao.getNormByPage(queryMap, start, size);
        //构造createUser updateUser名字，如果列表页面不显示，以下代码可以去掉
        if (null != list && list.size() > 0) {
            for (ProductNorm norm : list) {
                SystemAdmin admin = systemAdminReadDao.get(norm.getCreateId());
                ...
                if (null != admin && norm.getCreateId() == norm.getUpdateId()) {
                    ...
                    admin = systemAdminReadDao.get(norm.getUpdateId());
                    if (null != admin && !StringUtil.isEmpty(admin.getName())) {
                        norm.setCreateUser(admin.getName());
                    }
                }
    ...
    public Boolean updateNorm(Map<String, Object> map) {
        ...
        try {
            ...
            List<ProductNormAttr> attrOlds = productNormReadDao.getAttrByNormId(norm.getId());
    ...
    public List<ProductType> chkHasUsed(Integer id) {
        List<ProductType> list = productTypeReadDao.getByNormId(id);
        //if (null != list && list.size() > 0)
        //    throw new BusinessException("该商品规格已经在商品类型中使用,不可进行编辑");
        return list;
    }
    ...
    public Boolean delNorm(Integer id) {
        ...
        //2. dbOper
        ProductNorm norm = productNormReadDao.getNormById(id);
    ...
    public Boolean saveNormAttr(ProductNormAttr attr) {
        ...
        ProductNorm norm = productNormReadDao.getNormById(attr.getProductNormId());
        if (null == norm)
    ...
    public ProductNormAttr getNormAttrById(Integer id) {
        ...
        return productNormReadDao.getNormAttrById(id);
    }

    public List<ProductNormAttr> getAttrByNormId(Integer id) {
        if (null == id || 0 == id)
            throw new BusinessException("获取商品规格属性失败，无效的商品规格id");
        return productNormReadDao.getAttrByNormId(id);
    }

    public List<ProductNormAttr> getAttrByNormIds(String ids) {
        if (StringUtil.isEmpty(ids))
            throw new BusinessException("获取商品规格属性失败，无效的商品规格id");
        return productNormReadDao.getAttrByNormIds(ids);
    }

    public List<ProductNormAttr> pageNormAttr(Map<String, String> queryMap, PagerInfo pager) {
        Integer start = 0, size = 0;
        if (pager != null) {
            pager.setRowsCount(productNormReadDao.countNormAttr(queryMap));
            start = pager.getStart();
            size = pager.getPageSize();
        }
        return productNormReadDao.getNormAttrByPage(queryMap, start, size);
    }
    ...
    public Boolean saveNormAttrOpt(ProductNormAttrOpt opt) {
        ...
        if (null == productReadDao.get(opt.getProductId()))
            throw new BusinessException("保存商品选定的规格属性失败，商品为空");
        if (null == sellerReadDao.get(opt.getSellerId()))
    ...
    public List<ProductNorm> listNoPage() {
        return productNormReadDao.listNoPage();
    }
}
