package com.ejavashop.model.settlement;
...
import com.ejavashop.dao.shop.read.settlement.SettlementReadDao;
import com.ejavashop.dao.shop.write.settlement.SettlementOpWriteDao;
import com.ejavashop.dao.shop.write.settlement.SettlementWriteDao;
...
public class SettlementModel {
    ...
    public boolean jobSettlement() {
        ...
        // 结算周期yyyyMM
        String settleCycle = String.valueOf(year) + String.valueOf(month);
        // 周期开始时间，1号0时0分0秒
        ...
        if (sellerList != null && sellerList.size() > 0) {
            for (Seller seller : sellerList) {
                ...
                try {
                     ...
                     * 订单网单相关金额计算
                     */
                    List<Orders> ordersList = ordersReadDao.getSettleOrders(seller.getId(), startTime, endTime);
                        ...
                        for (Orders orders : ordersList) {
                            ...
                            if (opList != null && opList.size() > 0) {
                                for (OrdersProduct op : opList) {
                                    ...
                                    // 网单佣金金额，四舍五入保留2位小数
                                    BigDecimal opCommision = op.getMoneyAmount().multiply(cate.getScaling());
                     ...
                     * 退货金额计算
                     */
                    List<MemberProductBack> backList = memberProductBackReadDao.getSettleBacks(seller.getId(), startTime, endTime, 0, 0);
                    ...
                    payable = ((moneyOrder.subtract(moneyBack)).subtract(commision)).subtract(integralMoneyBack);
    ...
    public Settlement getSettlementById(Integer settlementId) {
        return settlementReadDao.get(settlementId);
    }
