...
import com.ejavashop.dao.shop.read.member.MemberGradeIntegralLogsReadDao;
import com.ejavashop.dao.shop.read.member.MemberGradeUpLogsReadDao;
import com.ejavashop.dao.shop.read.member.MemberLoginLogsReadDao;
...
import com.ejavashop.dao.shop.write.member.MemberCollectionSellerWriteDao;
...
import com.ejavashop.dao.shop.write.member.MemberGradeUpLogsWriteDao;
import com.ejavashop.dao.shop.write.member.MemberLoginLogsWriteDao;
    ...
    */
    ...
    public boolean updateMemberValue(MemberGradeIntegralLogs logs) {
        ...
        try {
            ...
            if (logs.getOptType().intValue() == ConstantsEJS.MEMBER_GRD_INT_LOG_OPT_T_6) {
                Integer value = logs.getValue();
                logs.setValue(value - (value * 2));
            }

            // 取得会员
            Member member = memberReadDao.get(logs.getMemberId());
            if (member == null) {
            ...
            // 操作类型是经验值的情况
            if (ConstantsEJS.MEMBER_GRD_INT_LOG_T_1 == logs.getType().intValue()) {
                ...
                // 取得会员等级配置，判断会员等级
                MemberGradeConfig memberGradeConfig = memberGradeConfigReadDao
            ...
            } else if (ConstantsEJS.MEMBER_GRD_INT_LOG_T_2 == logs.getType().intValue()) {
                // 如果新的经验值是负数，说明经验值不够扣减，返回错误
     ...
     * @return
     */
    public Integer getMemberCollectionSellersCount(Integer memberId) {
        return memberCollectionSellerReadDao.getMemberCollectionSellersCount(memberId);
    }
     ...
     * @return
     */
    public List<MemberCollectionSeller> getMemberCollectionSellers(Integer memberId, Integer start,
                                                                   Integer size) {
        return memberCollectionSellerReadDao.getMemberCollectionSellers(memberId, start, size);
    }
     ...
     * @return
     */
    public Integer getMemberCollectionProductsCount(Integer memberId) {
        return memberCollectionProductReadDao.getMemberCollectionProductsCount(memberId);
    }
     ...
     * @return
     */
    public List<MemberCollectionProduct> getMemberCollectionProducts(Integer memberId,
                                                                     Integer start, Integer size) {
        return memberCollectionProductReadDao.getMemberCollectionProducts(memberId, start, size);
    }
     ...
     * @return
     */
    public boolean updateMemberBalance(MemberBalanceLogs logs, SystemAdmin systemAdmin) {
        // 事务管理
        DefaultTransactionDefinition def = new DefaultTransactionDefinition();
        def.setPropagationBehavior(TransactionDefinition.PROPAGATION_REQUIRES_NEW);
        TransactionStatus status = transactionManager.getTransaction(def);
        try {
            Assert.notNull(memberWriteDao, "Property 'memberWriteDao' is required.");
            Assert.notNull(memberBalanceLogsWriteDao, "Property 'memberBalanceLogsWriteDao' is required.");
            //  参数校验
            if (logs.getMemberId() == null) {
                throw new BusinessException("会员ID不能为空，请重试！");
            ...
            if (logs.getState().equals(MemberBalanceLogs.STATE_3)
                ...
                logs.setMoney(money.subtract(money.multiply(new BigDecimal(2))));
            }
...
//            }
            ...
            //赊账信息
            MemberCredit mc = memberCreditReadDao.getByMemberId(newMember.getId());

            //如果有赊账记录，则记录赊账日志
            if (mc != null) {
                BigDecimal money = logs.getMoney();
                BigDecimal usedMoney = mc.getUsed();
                BigDecimal surplus = mc.getSurplus();
                
                MemberCreditLog mcl = new MemberCreditLog();
                mcl.setCreditBefore(mc.getSurplus());
                
                //state = 5 系统增加， state = 6 系统减少
//                if (logs.getState().equals(MemberBalanceLogs.STATE_5)) {
//                    mc.setUsed(usedMoney.subtract(money));
//                }
//                else if (logs.getState().equals(MemberBalanceLogs.STATE_6)) {
//                    mc.setUsed(usedMoney.add(money));
//                }
                mc.setUsed(usedMoney.subtract(money));
                mc.setSurplus(surplus.add(money));
                mcl.setCreditAfter(surplus.add(money));
                memberCreditWriteDao.update(mc);
                
                mcl.setCreditId(mc.getId());
                mcl.setType(2);//系统调整
                mcl.setMemberId(newMember.getId());
                mcl.setBalanceBefore(member.getBalance());
                mcl.setBalanceAfter(logs.getMoneyAfter());
                mcl.setCreditAmount(logs.getMoney());
                //state = 5 系统增加， state = 6 系统减少
//                if (logs.getState().equals(MemberBalanceLogs.STATE_5)) {
//                    mcl.setCreditAmount(newMember.getBalance().subtract(member.getBalance()));
//                }
//                else if (logs.getState().equals(MemberBalanceLogs.STATE_6)) {
//                    mcl.setCreditAmount(newMember.getBalance().add(member.getBalance()));
//                }
                mcl.setCreateId(systemAdmin.getId());
                mcl.setCreateName(systemAdmin.getName());
                mcl.setCreatTime(new Date());
                memberCreditLogWriteDao.save(mcl);
            }
    ...
    public MemberRule getMemberRule(Integer memberRuleId, Integer state) {
        return memberRuleReadDao.get(memberRuleId, state);
    }
    ...
    public boolean memberRegistSendValue(Integer memberId, String memberName) throws Exception {
        ...
        try {
            ...
            memberGradeIntegralLogs.setOptType(ConstantsEJS.MEMBER_GRD_INT_LOG_OPT_T_1);
            ...
            if (!gradeValue.equals(0)) {
                ...
                // 记录送经验值的日志【会员经验积分日志表】
                memberGradeIntegralLogs.setType(ConstantsEJS.MEMBER_GRD_INT_LOG_T_1);
    ...
    public boolean memberLoginSendValue(Integer memberId, String memberName) throws Exception {
        ...
        try {
            ...
            List<MemberLoginLogs> logsList = memberLoginLogsReadDao.getByMemberIdAndTime(memberId,
                startTime, endTime);
            // 如果超过一次了则返回
            ...
            sendValue(memberId, memberName, gradeValue, integralValue, memberGradeConfig,
                ConstantsEJS.MEMBER_GRD_INT_LOG_OPT_T_2, "会员登录");
    ...
    public boolean memberOrderSendValue(Integer memberId, String memberName,
        ...
        try {
            ...
            sendValue(memberId, memberName, gradeValue, integralValue, memberGradeConfig,
                ConstantsEJS.MEMBER_GRD_INT_LOG_OPT_T_3, "订单" + orders.getOrderSn() + "送积分或经验值");
    ...
    public boolean memberEvaluateSendValue(Integer memberId, String memberName,
        ...
        try {
            ...
            if (memberId == null) {
                ...
                throw new BusinessException("商品ID不能为空，请重试！");
            }
            ...
            sendValue(memberId, memberName, gradeValue, integralValue, memberGradeConfig,
                ConstantsEJS.MEMBER_GRD_INT_LOG_OPT_T_4, "评论商品" + productId + "送积分");
